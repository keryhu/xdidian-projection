...
import com.xdidian.keryhu.email_activate.service.ConverterUtil;
import com.xdidian.keryhu.email_activate.service.ResendService;
import com.xdidian.keryhu.email_activate.stream.ResendProducer;
...
class ResendServiceImpl implements ResendService {
    ...
    if (clickCoolingExpired(email)) {
      ...
      if (!sendMoreThanLimit(email)) {
        ...
        repository.findByEmail(email).ifPresent(e -> {
          ...
          e.setSendExpiryDate(
              LocalDateTime.now().plusMinutes(activatedProperties.getSend_cycle_minutes()));
          repository.save(e);
  ...
  /**
   * 点击“重新发送”url的冷却时间，也就是客户不能连续点击 该url 每次客户点击完url，开始计时，目前设定的是2分钟，只有等过了2分钟，客户才能继续点击此url
   * 如果email不存在，则默认返回false，即假如email都不存在，那么肯定不能点击此url)
   */
  private boolean clickCoolingExpired(String email) {
    return repository.findByEmail(email).map(e -> e.getSendExpiryDate() == null ? true
        : LocalDateTime.now().isAfter(e.getSendExpiryDate())).orElse(false);
  }


  /**
   * 查看当前email，点击“重新发送激活邮件的次数有没有超过限制 如果email不存在，肯定不让点击此url，所以默认是true
   */
  private boolean sendMoreThanLimit(String email) {
    return repository.findByEmail(email)
        .map(e -> e.getSentTimes() >= activatedProperties.getMax_send_times()).orElse(true);
  }
