...
/**
 * Description : LoginAttemptUserService interface 具体的 实现类
 * Date : 2016年06月17日 下午10:26
 * Author : keryHu keryhu@hotmail.com
 */
...
public class LoginAttemptUserServiceImpl implements LoginAttemptUserService {
    ...
    /**
     * 用户登陆失败，促发的事件，并且传递参数用户ip，和userid，此种情况是userId不为空</p>
    ...
    /**
     * 用户登陆成功后，所做的具体事情，登录成功，肯定能找的用户userId，所以就纪录数据库
     * 如果ip不存在，则无需做任何事情，
     * ip存在的情况下，恢复默认数据
     */
    @Override
    ...
    /**
     * isBlocked 根据当前ip判断当前远程用户是否处于被锁定24小时状态。
