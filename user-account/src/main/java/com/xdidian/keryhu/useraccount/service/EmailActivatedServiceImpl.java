...
public class EmailActivatedServiceImpl implements EmailActivatedService {
	 ...
	 * Description: 查询当前loginName所在的user 数据库账户，email 有没有被激活
	 * @param loginName
	 * @return 返回默认fasle
	...
	public boolean emailActivatedStatus(String loginName) {
		...
		return userService.findByLoginName(loginName).map(e -> e.isEmailActivatedStatus()).orElse(false);
	}

	/**
	 * Title: emailActivatedExpired
	 * Description: 通过登录名（email或phone）查看数据库的user，如果email 存在的情况下，查看注册时间 是否 已经大于 最大的允许时间
	...
	public boolean emailActivatedExpired(String loginName) {
		...
		return userService.findByLoginName(loginName)
		                 ...
		                 .orElseThrow(()->new EmailNotFoundException("您需要查询的email或phone不存在于数据库"));	
	}

     /**	
	 * Title: emailActivatedCodeExist
	 * Description:通过loginName登录名，查看数据库中user，默认false。如果email不存在，返回false，如果 查询的对象，不存在emailActivatedCode，则返回false
	...
	public boolean emailActivatedCodeExist(String loginName, String emailActivatedCode) {
		// TODO Auto-generated method stub
				return userService.findByLoginName(loginName).map(e -> {
	...
	*/ 
	...
	public void retryEmailActivated(String loginName) {
		...
		userService.findByLoginName(loginName).ifPresent(e->{
			e.setEmailActivatedCode(randomUtil.create(emailActivatedProperties.getCodeLength()));
